...
class MYPDF extends TCPDF {
	...
	public function Header() {
		...
		// Title
		$this->Cell(0, 30, '產品進貨彙總表', 0, false, 'C', 0, '', 0, false, 'C', 'B');
...
$tbl_header = 
	'<table cellpadding="5" border="1">'.
		...
		'<tr bgcolor="#F2F2F2">'.
			...
			'<td align="right">品項單價</td>'.
		'</tr>';
$tbl_footer = '</table>';
$tbl = '';

$sql = 
"SELECT P.ProductPurchaseID, C.CompanyName, P.SetupDate ".
"From ProductPurchases P, Companies C, Employees E ".
"WHERE ";
if(!is_null($CondField)) {
	$sql = $sql."P.".$CondField."=".$CondValue." AND ";
}
if(!is_null($LikeField)) {
	$sql = $sql.$LikeField." Like '%".$LikeKeyword."%' AND ";
}
if(!is_null($DateAfterValue)) {
	$sql = $sql." P.SetupDate >= '".$DateAfterValue."' AND ";
}
if(!is_null($DateBeforeValue)) {
	$sql = $sql." P.SetupDate <= '".$DateBeforeValue."' AND ";
}
$sql = $sql.
...
foreach ($resp as $value) {
	$ProductPurchaseID = $value['ProductPurchaseID'];
	$CompanyName = $value['CompanyName'];
	$SetupDate = $value['SetupDate'];
	
	$Detailsql = 
	"SELECT IPD.ProductPurchaseDetailID, P.ProductID, P.Name As ProductName, IPD.UnitPrice, IPD.ActualQuantity, P.Unit ".
	"FROM ProductPurchaseDetails IPD, Products P ".
	"WHERE IPD.ProductPurchaseID = '$ProductPurchaseID' AND ".
	"IPD.ProductID = P.ProductID AND ".
	"IPD.State = 1 ".
	"ORDER BY IPD.ProductPurchaseDetailID ASC ;";
	...
	foreach ($Detailresp as $Detailvalue) {
		...
		$UnitPrice = $Detailvalue['UnitPrice'];
		if($i++ == 0) {
			$tbl = $tbl.
			'<tr>'.
		...
		$Sum = $ActualQuantity * $UnitPrice;
		if(isset($ItemSum[$ProductID]['Quantity'])) {
			$ActualQuantity += $ItemSum[$ProductID]['Quantity'];
		}
		if(isset($ItemSum[$ProductID]['Sum'])) {
			$Sum += $ItemSum[$ProductID]['Sum'];
		}
		$ItemSum[$ProductID] = array(

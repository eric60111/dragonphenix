...
class MYPDF extends TCPDF {
	...
	public function Header() {
		...
		// Title
		$this->Cell(0, 30, '配方料進貨彙總表', 0, false, 'C', 0, '', 0, false, 'C', 'B');
...
$sql = 
"SELECT M.MaterialPurchaseID, C.CompanyName, M.SetupDate ".
"From MaterialPurchases M, Companies C, Employees E ".
"WHERE ";
...
foreach ($resp as $value) {
	$MaterialPurchaseID = $value['MaterialPurchaseID'];
	$CompanyName = $value['CompanyName'];
	$SetupDate = $value['SetupDate'];
	
	$Detailsql = 
	"SELECT IPD.MaterialPurchaseDetailID, M.MaterialID, M.Name As MaterialName, IPD.UnitPrice, IPD.ActualQuantity, M.Unit ".
	"FROM MaterialPurchaseDetails IPD, Materials M ".
	"WHERE IPD.MaterialPurchaseID = '$MaterialPurchaseID' AND ".
	"IPD.MaterialID = M.MaterialID AND ".
	"IPD.State = 1 ".
	"ORDER BY IPD.MaterialPurchaseDetailID ASC ;";
...
ob_clean();
$pdf->Output($_SERVER['DOCUMENT_ROOT'].'MaterialPurchase.pdf', 'I');

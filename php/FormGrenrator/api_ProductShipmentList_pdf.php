...
class MYPDF extends TCPDF {
	...
	public function Header() {
		...
		// Title
		$this->Cell(0, 30, '產品出貨彙總表', 0, false, 'C', 0, '', 0, false, 'C', 'B');
...
$sql = 
"SELECT P.ProductShipmentID, C.CompanyName, P.SetupDate ".
"From ProductShipments P, Companies C, Employees E ".
"WHERE ";
...
foreach ($resp as $value) {
	$ProductShipmentID = $value['ProductShipmentID'];
	$CompanyName = $value['CompanyName'];
	$SetupDate = $value['SetupDate'];
	
	$Detailsql = 
	"SELECT IPD.ProductShipmentDetailID, P.ProductID, P.Name As ProductName, IPD.SoldPrice, IPD.ActualQuantity, P.Unit ".
	"FROM ProductShipmentDetails IPD, Products P ".
	"WHERE IPD.ProductShipmentID = '$ProductShipmentID' AND ".
	"IPD.ProductID = P.ProductID AND ".
	"IPD.State = 1 ".
	"ORDER BY IPD.ProductShipmentDetailID ASC ;";
...
ob_clean();
$pdf->Output('ProductShipment.pdf', 'I');
